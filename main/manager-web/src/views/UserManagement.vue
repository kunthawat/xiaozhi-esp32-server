...
export default {
  ...
  methods: {
    ...
    batchDelete() {
      ...
      this.$confirm(`Are you sure you want to delete the selected ${selectedUsers.length} users?`, "Warning", {
          ...
          try {
            ...
            if (failCount === 0) {
              ...
              this.$message.warning(
                `Successfully deleted ${successCount} users, ${failCount} failed to delete`
              );
            }
        ...
        .catch(() => {
          this.$message.info("Delete operation canceled");
        });
    },
    batchEnable() {
      const selectedUsers = this.userList.filter(user => user.selected);
    ...
    resetPassword(row) {
      this.$confirm("A new password will be generated after reset. Do you want to continue?", "Tip", {
        confirmButtonText: "Confirm",
        cancelButtonText: "Cancel",
      }).then(() => {
        Api.admin.resetUserPassword(row.userid, ({ data }) => {
          if (data.code === 0) {
            ...
            this.$message.success({
              message: "Password has been reset. Please notify the user to use the new password to log in",
              showClose: true
            });
          }
        });
    ...
    handleChangeStatus(row, status) {
      ...
      this.$confirm(`Are you sure you want to ${confirmText} the selected ${userCount} user(s)?`, 'Tip', {
        confirmButtonText: 'Confirm',
        cancelButtonText: 'Cancel',
        type: 'warning'
      }).then(() => {
        ...
        Api.user.changeUserStatus(status, userIds, ({ data }) => {
          if (data.code === 0) {
            this.$message.success({
              message: `Successfully ${confirmText}d ${userCount} user(s)`,
              showClose: true
            });

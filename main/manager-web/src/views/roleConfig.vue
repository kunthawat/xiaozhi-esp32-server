                ...
                <div class="form-grid">
                  ...
                  <div class="form-column">
                    <div class="model-row">
                      ...
                      <el-form-item label="Automatic Speech Recognition (ASR)" class="model-item">
                        <div class="model-select-wrapper">
...
export default {
  ...
  data() {
    return {
      ...
      models: [
        ...
        { label: 'Vision Large Language Model (VLLM)', key: 'vllmModelId', type: 'VLLM' },
        { label: 'Intent Recognition', key: 'intentModelId', type: 'Intent' },
  ...
  methods: {
    ...
    saveConfig() {
      ...
      Api.agent.updateAgentConfig(this.$route.query.agentId, configData, ({ data }) => {
        if (data.code === 0) {
          ...
          this.$message.error({
            message: data.msg || 'Configuration save failed',
            showClose: true
          });
        }
      });
    ...
    resetConfig() {
      this.$confirm('Are you sure you want to reset the configuration?', 'Confirmation', {
    ...
    fetchAgentConfig(agentId) {
      Api.agent.getDeviceConfig(agentId, ({ data }) => {
        if (data.code === 0) {
          ...
          // First ensure allFunctions is loaded (if not, fetch allFunctions first)
          const ensureFuncs = this.allFunctions.length
            ? Promise.resolve()
            : this.fetchAllFunctions();

          ensureFuncs.then(() => {
            // Merge: supplement complete metadata information according to pluginId (id field)
            ...
            // Backup original for restoration when cancelled
    ...
    updateIntentOptionsVisibility() {
      ...
      // If current selected intent recognition is function_call but LLM type doesn't support it, set to first available option

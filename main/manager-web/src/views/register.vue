        ...
        <div class="login-box">
          ...
          <div style="padding: 0 30px;">
            <form @submit.prevent="register">
              ...
              <div class="input-box">
                <img loading="lazy" alt="" class="input-icon" src="@/assets/login/password.png" />
                <el-input v-model="form.confirmPassword" placeholder="Please confirm password" type="password" show-password />
              </div>

              <!-- Keep verification code section the same -->
  ...
  mounted() {
    this.$store.dispatch('fetchPubConfig').then(() => {
      if (!this.allowUserRegister) {
        showDanger('Regular user registration is currently not allowed');
        setTimeout(() => {
          goToPage('/login');
        }, 1500);
      }

  ...
  <div class="welcome">
    ...
    <div class="operation-bar">
      ...
      <div class="right-operations">
        <el-input placeholder="Enter device model or Mac address" v-model="searchKeyword" class="search-input"
        ...
        <div class="content-area">
          <el-card class="device-card" shadow="never">
            <el-table ref="deviceTable" :data="paginatedDeviceList" class="transparent-table"
              ...
              <el-table-column label="Binding Time" prop="bindTime" align="center"></el-table-column>
              ...
              <el-table-column label="OTA Update" align="center">
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    deleteSelected() {
      ...
      this.$confirm(`Are you sure you want to unbind ${this.selectedDevices.length} selected devices?`, 'Warning', {
        ...
        return new Promise((resolve, reject) => {
          Api.device.unbindDevice(id, ({data}) => {
            if (data.code === 0) {
              ...
              reject(data.msg || 'Unbinding failed');
            }
          });
        });
      });
      Promise.all(promises)
          .then(() => {
            this.$message.success({
              message: `Successfully unbound ${deviceIds.length} devices`,
              showClose: true
            });
    ...
    submitRemark(row) {
      ...
      if (text.length > 64) {
        this.$message.warning('Remarks cannot exceed 64 characters');
        return;
      }
      ...
      this.updateDeviceInfo(row.device_id, { alias: text }, (ok, resp) => {
        if (ok) {
          ...
          this.$message.success('Remarks saved');
        } else {
          row.remark = row._originalRemark;
          this.$message.error(resp.msg || 'Failed to save remarks');
        }
    ...
    handleOtaSwitchChange(row) {
      this.updateDeviceInfo(row.device_id, {autoUpdate: row.otaSwitch ? 1 : 0}, (result, {msg}) => {
        if (result) {
          this.$message.success(row.otaSwitch ? 'Auto-update enabled' : 'Auto-update disabled');
          return;
        }

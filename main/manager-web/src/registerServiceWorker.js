    ...
    window.addEventListener('load', () => {
      ...
      navigator.serviceWorker.getRegistrations().then(registrations => {
        ...
        navigator.serviceWorker
          .register(swUrl)
          .then(registration => {
            console.info('[Xiaozhi Service] Service Worker registration successful');
              ...
              installingWorker.onstatechange = () => {
                if (installingWorker.state === 'installed') {
                  if (navigator.serviceWorker.controller) {
                    // Content has been cached for updates, notify user to refresh
                    console.log('[Xiaozhi Service] New content available, please refresh the page');
                    // Can display update notification here
                    ...
                    // Everything is normal, Service Worker has been successfully installed
                    ...
                    setTimeout(() => {
                      ...
                      cdnUrls.forEach(url => {
                        fetch(url, { mode: 'no-cors' }).catch(err => {
                          console.log(`Failed to warm up cache for ${url}`, err);
                        });
                      });
                    }, 2000);
                  }
          ...
          .catch(error => {
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to register a ServiceWorker')) {
              ...
              if (process.env.NODE_ENV === 'production') {
                console.info(
                  'Possible reasons: 1. Server not configured with correct MIME type 2. Server SSL certificate issues 3. Server not returning service-worker.js file'
                );
              }
            }
          });

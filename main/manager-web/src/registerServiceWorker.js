  ...
  if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      ...
      navigator.serviceWorker.getRegistrations().then(registrations => {
        ...
        // Continue to register Service Worker
        navigator.serviceWorker
              ...
              installingWorker.onstatechange = () => {
                if (installingWorker.state === 'installed') {
                  if (navigator.serviceWorker.controller) {
                    // Content has been cached and updated, notify user to refresh
                    ...
                    // Everything is normal, Service Worker installed successfully
                    ...
                    setTimeout(() => {
                      ...
                      cdnUrls.forEach(url => {
                        fetch(url, { mode: 'no-cors' }).catch(err => {
                          console.log(`Cache warm-up failed for ${url}`, err);
                        });
                      });
                    }, 2000);
                  }
          ...
          .catch(error => {
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to register a ServiceWorker')) {
              console.warn('[XiaoZhi Service] Network error occurred while registering Service Worker, CDN resources may not be cached');
              if (process.env.NODE_ENV === 'production') {
                console.info(
                  'Possible causes: 1. Server not configured with correct MIME type 2. Server SSL certificate issues 3. Server not returning service-worker.js file'
                );
              }
            }
          });

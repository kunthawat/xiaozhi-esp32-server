    ...
    <div class="dialog-content">
      <el-form :model="deviceForm" :rules="rules" ref="deviceForm" label-width="100px">
        ...
        <el-form-item label="Firmware Ver." prop="appVersion">
          <el-input v-model="deviceForm.appVersion" placeholder="Enter firmware version"></el-input>
        </el-form-item>
        <el-form-item label="Mac Address" prop="macAddress">
          <el-input v-model="deviceForm.macAddress" placeholder="Enter Mac address"></el-input>
        </el-form-item>
      </el-form>
    </div>
...
export default {
  ...
  data() {
    ...
    const validateMac = (rule, value, callback) => {
      ...
      if (!value) {
        callback(new Error('Please enter a MAC address'));
      } else if (!macRegex.test(value)) {
        callback(new Error('Please enter a valid MAC address format, e.g.: 00:1A:2B:3C:4D:5E'));
      } else {
        callback();
      }
    };

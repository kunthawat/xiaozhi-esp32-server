...
export default {
  ...
  data() {
    return {
      saving: false,
      rules: {
        modelType: [{ required: true, message: 'Please select a category', trigger: 'change' }],
      ...
      tableKey: 0 // Used to force table re-rendering
    };
  },
  computed: {
    hasIncompleteFields() {
  ...
  methods: {
    ...
    forceTableRerender() {
      this.tableKey += 1; // Change key value to force table re-rendering
    },

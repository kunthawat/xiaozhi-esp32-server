  ...
  methods: {
    ...
    handleSearch() {
      ...
      try {
        ...
        // Trigger search event, pass the regular expression to parent component
        this.$emit('search', regex);
      } catch (error) {
        console.error('Regular expression creation failed:', error);
        this.$message.error({
          message: 'Search keyword format is incorrect',
          showClose: true
        });
      }
    },
    ...
    // Monitor the visibility change of the second dropdown menu

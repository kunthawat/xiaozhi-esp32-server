...
// TerserPlugin for JavaScript compression
...
module.exports = defineConfig({
  productionSourceMap: process.env.NODE_ENV !=='production', // Don't generate source map in production environment
  devServer: {
    ...
    client: {
      overlay: false, // Don't show webpack error overlay
    },
  },
  ...
  configureWebpack: config => {
    if (process.env.NODE_ENV === 'production') {
      ...
      // Decide whether to add Service Worker based on CDN usage
      config.plugins.push(
        new InjectManifest({
      ...
      if (useCDN) {
        ...
        // Ensure externals are not set when not using CDN, let webpack bundle all dependencies
        config.externals = {};
      }

      if (process.env.ANALYZE === 'true') {  // Control through environment variables
        config.plugins.push(

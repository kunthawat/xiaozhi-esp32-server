...
class AsyncVisionPerformanceTester:
    ...
    async def _test_vllm(self, vllm_name: str, config: Dict) -> Dict:
        """Asynchronously test performance of a single vision large model"""
        try:
            ...
            if len(filtered_times) < len(test_tasks) * 0.5:
                print(f"⚠️  {vllm_name} insufficient valid data, possible network instability")
                return {"name": vllm_name, "type": "vllm", "errors": 1}
    ...
    async def run(self):
        """Execute comprehensive asynchronous testing"""
        print("🔍 Starting to filter available vision large models...")

        if not self.test_images:
            print(f"\n⚠️  No image files found in {self.image_root} path, unable to test")
            return

...
hass_get_state_function_desc = {
    ...
    "function": {
        "name": "hass_get_state",
        "description": "Get device status in Home Assistant, including querying light brightness, color, color temperature, media player volume, device pause/resume operations",
        "parameters": {
            "type": "object",
            "properties": {
                "entity_id": {
...
def hass_get_state(conn, entity_id=""):
    try:
        ...
        logger.bind(tag=TAG).error("Home Assistant status query timeout")
        return ActionResponse(Action.ERROR, "Request timeout", None)
    except Exception as e:
        error_msg = f"Home Assistant operation failed"
